...
# HOY-222 / HOY-261: brand the payload's piConfig. Pi reads this payload copy at
# runtime via PI_PACKAGE_DIR and derives from it:
#   configDir (.pi -> .hoy)  every project-dir path (settings, packages, skills,
#                            prompts, trust, extensions)
#   name      (pi -> hoy)    APP_NAME, hence the env var Pi honors for the agent
#                            dir: PI_CODING_AGENT_DIR -> HOY_CODING_AGENT_DIR
#                            (config.js ENV_AGENT_DIR = `${APP_NAME}_CODING_AGENT_DIR`).
#                            sidecar.rs and hoy-sidecar.ts export/read that name.
# We edit only the payload copy that ships, not node_modules (keeps npm ci clean).
bun -e 'const f=process.argv[1]; const fs=require("fs"); const p=JSON.parse(fs.readFileSync(f)); p.piConfig={...p.piConfig,configDir:".hoy",name:"hoy"}; fs.writeFileSync(f, JSON.stringify(p,null,2))' "$PAYLOAD/package.json"
# Fail loud if a future Pi package.json shape change drops either override.
...
APP_NAME="$(bun -e 'process.stdout.write(String(JSON.parse(require("fs").readFileSync(process.argv[1])).piConfig?.name))' "$PAYLOAD/package.json")"
if [ "$APP_NAME" != "hoy" ]; then
  echo "ERROR: payload piConfig.name is '$APP_NAME', expected hoy (HOY-261; drives HOY_CODING_AGENT_DIR)" >&2
  exit 1
fi

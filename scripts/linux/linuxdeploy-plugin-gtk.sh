...
# Vendored from https://github.com/tauri-apps/linuxdeploy-plugin-gtk with a
...
# runtime. Both are avoided by sheltering the sidecar out of usr/bin/ before
...
#
# Pre-staged into ~/.cache/tauri/ before the build so Tauri uses it instead of
# downloading the upstream plugin. Adapted from khoj-ai/pipali.
#
# See: https://github.com/linuxdeploy/linuxdeploy/issues/147
#      https://github.com/tauri-apps/tauri/issues/14796
#      https://github.com/tauri-apps/tauri/issues/15106
...
# PATCH (HOY-217): shelter sidecar/static binaries out of AppDir before
# linuxdeploy scans usr/bin/. A binary is sheltered when `ldd` fails on it
# (linuxdeploy would crash) or when its name is in LINUXDEPLOY_SHELTER_BINS
# (our self-contained sidecar, which must not be rpath-rewritten). Restored
# pristine after linuxdeploy runs.
...
while IFS= read -r -d '' file; do
    ...
    if ! ldd "$file" > /dev/null 2>&1; then
        ...
        for name in ${LINUXDEPLOY_SHELTER_BINS:-}; do
            if [ "$base" = "$name" ]; then shelter=true; break; fi
        done
    fi
    if [ "$shelter" = true ]; then
        echo "Temporarily moving sidecar/static binary out of AppDir: $base"
...
# PATCH: Restore sheltered binaries
